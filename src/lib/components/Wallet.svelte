<script lang="ts" type="module">
	...
	async function getBalances() {
		try {
			...
			$scrtBalance = Number((response.balance?.amount as any) / 1e6).toFixed(6).toString();
		} catch (error) {
			console.log(error);
		}
...
<div class="card !ring-primary-500 p-4 w-64 shadow-2xl transition-opacity" data-popup="walletMenu">
...
<div class="flex flex-row-reverse flex-nowrap md:gap-4 items-center">
	...
	{:else}
		<button class="btn font-bold variant-ghost-primary" on:click={() => connect()}>
			<svg
				viewBox="0 0 21 18"
				fill="none"
				xmlns="http://www.w3.org/2000/svg"
