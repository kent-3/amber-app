...
export async function connectLedger(transport = 'usb') {
	const trans =
		transport === 'usb' ? await TransportWebUSB.request() : await TransportWebBLE.create();
	console.log(trans);
...
export function consensusPubkeyToHexAddress(consensusPubkey: any) {
	...
	if (typeof consensusPubkey === 'object') {
		if (consensusPubkey['@type'] === '/cosmos.crypto.ed25519.PubKey') {
			...
			raw = toHex(sha256(fromBase64(consensusPubkey.key)))
				.slice(0, 40)
				.toUpperCase();
			return raw;
		}
		...
		if (consensusPubkey['@type'] === '/cosmos.crypto.secp256k1.PubKey') {
			raw = new RIPEMD160()
				.update(Buffer.from(sha256(fromBase64(consensusPubkey.key))))
				.digest('hex');
			return raw;
		}
		...
		raw = sha256(
			fromHex(toHex(fromBech32(consensusPubkey).data).toUpperCase().replace('1624DE6420', ''))
		);
	}
...
async function getLedgerAppName(coinType: number, device: string | Transport, hdpath: HdPath) {
	...
	const transport = await (device === 'ledgerBle'
...
export async function getLedgerAddress(transport = 'blu', hdPath = "m/44'/529/0'/0/0") {
	const protocol =
		transport === 'usb' ? await TransportWebUSB.create() : await TransportWebBLE.create();
